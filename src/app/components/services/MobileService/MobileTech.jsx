...
export default function MobileTech() {
  ...
  return (
    // Caveman remove overflow-x-hidden from here to let sticky work
    <section className="relative w-full bg-slate-950 py-20 text-slate-100 sm:py-24 lg:py-28">
      {/* Background Glows & Pattern */}
      {/* Glows moved inside or made smaller to prevent overflow scrollbar */}
      ...
      <div className="relative z-10 w-full px-6 sm:px-12 md:px-16 lg:px-20 xl:px-28">
        {/* Header */}
        <div className="mb-16 flex flex-col items-start max-w-4xl">
          ...
          <p className="mt-4 text-base text-slate-400 sm:text-lg">
            We combine modern development frameworks, powerful programming languages, cloud platforms, and proven technologies to build secure, scalable, high-performance mobile applications ready for evolving business and digital needs.
          </p>

          <div className="mt-8 h-px w-full bg-slate-800/80" />
        </div>

        {/* Tech Groups - Sticky Stacking Stair Step Layout */}
        <div className="relative flex w-full flex-col gap-12 pb-16">
          {techGroups.map((group, groupIdx) => (
            <div
              key={groupIdx}
              className={`sticky ${group.topClass} ${group.zIndex} box-border flex flex-col w-full rounded-3xl border border-slate-800 bg-slate-900/95 p-6 md:p-8 shadow-2xl backdrop-blur-xl transition-all duration-300 hover:border-blue-500/40`}
            >
              <div className="mb-6 flex items-center justify-between border-b border-slate-800/80 pb-4">
                <h3 className="text-base font-bold uppercase tracking-widest text-blue-400 sm:text-lg">
                  {group.title}
                </h3>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                {group.items.map((item, itemIdx) => (
                  <div
                    key={itemIdx}
                    className="group/card relative flex items-center gap-5 overflow-hidden rounded-2xl border border-slate-800/80 bg-slate-950/80 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-blue-500/50 hover:bg-slate-950 hover:shadow-xl hover:shadow-blue-500/10"
                  >
                    <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-blue-500/10 opacity-0 blur-xl transition-opacity duration-300 group-hover/card:opacity-100" />

                    {/* Left Side: Large Icon */}
                    <div className="relative flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-slate-900/90 p-3.5 border border-slate-800 transition-all duration-300 group-hover/card:scale-105 group-hover/card:border-blue-500/40 group-hover/card:bg-slate-800/90 shadow-lg">
                      <Image
                        src={item.icon}
                        alt={item.name}
                        width={80}
                    ...
                    <div className="flex flex-col justify-center">
                      <h4 className="text-lg font-bold text-slate-100 transition-colors duration-300 group-hover/card:text-blue-400 mb-1">
                        {item.name}
                      </h4>
